...
$("#table").jqGrid({
    datatype: "local",
    colNames: ['仓储ID','商品ID','串码详情','操作', '商品信息<span class="sm_false" onclick="set_sm(this);">扫码枪</span>','所属仓库','当前库存','辅助属性','商品品牌','商品编号','商品分类','规格型号','条形码','商品单位','库存预警','商品货位','所需积分','零售名称','商品批次','商品串码','数量','总积分','备注信息'],
    colModel: [
        {
            name: 'set_id',
            hidden:true
...
function add_goods(arr,page_type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        if(page_type){
            ...
            rows_info['integral'] = arr[i].integral; //所需积分
            rows_info['nums'] = '1'; //默认数量
            rows_info['allintegral'] = arr[i].integral; //总积分
        }else{
            //详情
            ...
            rows_info['integral'] = arr[i].integral; //所需积分
            rows_info['allintegral'] = arr[i].allintegral; //总积分
    ...
    for (var i = 0; i < arr.length; i++) {
        if(arr[i].set_id !== '-1'){
            ...
            if(!regular_positive.test(arr[i].nums)){
                ...
                cal_row(arr[i].id,arr[i].nums,arr[i].integral);//计算行数据
                return false;
...
//计算表格数据
function cal_row(row_id,row_nums,row_integral,row_discount){
    var allintegral=cal((row_nums-0)*(row_integral-0));//单积分乘数量
    $("#table").jqGrid('setCell',row_id,'allintegral',allintegral);//设置单商品总积分
    var set_nums = cal($("#table").getCol('nums', false, 'sum')); //数量合计
    var set_allintegral = cal($("#table").getCol('allintegral', false, 'sum')); //积分合计
    $('#order_integral').val(set_allintegral); //单据积分
    $("#table").footerData("set", {"nums": "总数量:"+set_nums,"allintegral": "总积分:"+set_allintegral});//设置合计
}
...
function cal_default_rows(){
    ...
    var set_allintegral = cal($("#table").getCol('allintegral', false, 'sum')); //积分合计
    $('#order_integral').val(set_allintegral); //单据积分
    $("#table").footerData("set", {"nums": "总数量:"+set_nums,"allintegral": "总金额:"+set_allintegral});//设置合计
