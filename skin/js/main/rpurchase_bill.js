$(function(){
    layui.use('table', function() {
        var ape_cols=[
            ...
            {field: 'money', title: '实付金额', width: 150, align:'center'},
            ...
            {fixed: 'right',field: 'set', title: '相关操作', width: 150, align:'center',templet: '<div><div class="layui-btn-group"><div class="layui-btn-group"><button class="layui-btn layui-btn-primary layui-btn-sm" onclick="form_info({{d.id}})"><i class="layui-icon">&#xe912;</i></button> <button class="layui-btn layui-btn-primary layui-btn-sm" onclick="set_bill({{d.id}})"><i class="layui-icon">&#xe614;</i></button></div></div></div>'}
        ];//表格选项
        layui.table.render({
            id: 'ape_table',
            size: 'lg',
            ...
            even: ape_even,
            cols:  [ape_cols],
            url: '/index/service/rpurchasebill_list',
            page: true,
...
function export_data() {
    ...
    alert_info('稍等，数据请求中', "/index/export/rpurchase_bill?" + url_info,true);
}
//报表
function form_info(id) {
    ...
    layui.use('form', function() {
        layer.open({
            ...
            offset: '2%',
            area: ['98%', '96%'],
            content: '/index/main/rpurchase_info?id=' + id,
            end: function() {
        ...
        layer.ready(function() {
            var index = layer.open({
                ...
                success: function(layero, index) {
                    ...
                    //弹出后回调
                    $.post("/index/service/rpurchasebill_info", {
                        "id": id,
                    }, function(re) {
                        $('#actual').val(re.class.actual);
                        $('#money').val(re.class.money);
                        for (var i = 0; i < re.bill.length; i++) {
                            $('#tabs').append('<tr><td>'+re.bill[i].time+'</td><td>'+re.bill[i].account.info.name+'</td><td>'+re.bill[i].money+'</td><td>'+re.bill[i].user.info.name+'</td><td>'+re.bill[i].data+'</td><td><button class="layui-btn layui-btn-primary layui-btn-sm" onclick="del_rpurchasebill('+re.bill[i].id+','+re.bill[i].auth_info+')"><i class="layui-icon">&#xe640;</i></button></td></tr>');
                        }
                    });
                ...
                btn1: function(layero) {
                    ...
                    if(!regular_price.test(sum) || sum==="0"){
                        ...
                        $.post("/index/service/save_rpurchasebill", {
                            "id": id,
                            "account": account,
                            "sum": sum,
                            "data": data
...
//删除资金详情
function del_rpurchasebill(id,auth_type){
    if(auth_type){
        layui.use('layer', function() {
            layer.confirm('删除后资金将归还到原先账户，确定操作？', {
                ...
                $.post("/index/service/del_rpurchasebill", {
                    "id": id
                }, function(re) {
                    if (re === "success") {
                        re_alert('删除成功!');
