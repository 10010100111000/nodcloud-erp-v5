    ...
    layui.use('element', function() {
        ...
        var sn = setInterval(function(){
            if(nums==arr.length){
                ...
                $.post("/index/service/cal_summary", arr[nums], function(re) {
                    element.progress('ape', cal((100/arr.length)*nums)+'%');//更新进度
                    set_textarea('共需更新'+form_nums+'个报表,当前第'+(re.classcur)+'个['+re.name+']['+re.start+'-'+re.end+']。');
                }).error(function() {
                    clearInterval(sn);
                    set_textarea(' :( 服务器响应超时，请重新执行初始化操作。');
                });
                nums++;
            }
        }, 5130);
