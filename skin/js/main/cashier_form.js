$(function(){
    layui.use('table', function() {
        var ape_cols=[
            ...
            {fixed: 'right',field: 'set', title: '相关操作', width: 250, align:'center',templet: '<div><div class="layui-btn-group"><button class="layui-btn layui-btn-primary layui-btn-sm" onclick="cashier_min_print({{d.id}})"><i class="layui-icon">&#xe913;</i></button><button class="layui-btn layui-btn-primary layui-btn-sm" onclick="cashier_print({{d.id}})"><i class="layui-icon">&#xe911;</i></button> <button class="layui-btn layui-btn-primary layui-btn-sm" onclick="cashier_info({{d.id}})"><i class="layui-icon">&#xe912;</i></button> <button class="layui-btn layui-btn-primary layui-btn-sm" onclick="del_cashier({{d.id}},{{d.type.ape}})"><i class="layui-icon">&#xe640;</i></button></div></div>'}
        ];//表格选项
        layui.table.render({
            id: 'ape_table',
            size: 'lg',
            ...
            even: ape_even,
            cols:  [ape_cols],
            url: '/index/service/cashier_list',
            page: true,
...
function export_info(type){
    ...
    alert_info('稍等，数据请求中',"/index/export/cashier?"+url_info+"&mode="+type,true);
}

//打印
function cashier_print(id){
    //iframe层                                                                                                                                          
    layui.use('form', function(){
        layer.open({
          ...
          shadeClose: true,
          area: ['600px', '350px'],
          content: '/index/main/cashier_print?id='+ id
        }); 
    }); 
}
//修改
function cashier_info(id){
    //iframe层
    layui.use('form', function(){
        layer.open({
          type: 2,
          ...
          offset: '2%',
          area: ['98%', '96%'],
          content: '/index/main/cashier_info?id='+ id,
          end:function(){
        ...
        if(type){
            ...
            layui.use('layer', function() {
                layer.confirm('单据删除后不可恢复，确定删除？', {
                    ...
                    $.post("/index/service/del_cashier", {
                        "id": id
                    }, function(re) {
                        if (re === "success") {
                            re_alert('单据删除成功!');
...
function cashier_min_print(id){
    ...
    layui.use('form', function(){
        layer.open({
          ...
          shadeClose: true,
          area: ['600px', '350px'],
          content: '/index/main/cashier_min_print?id='+ id
        }); 
    }); 
}