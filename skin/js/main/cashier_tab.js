...
$("#table").jqGrid({
    datatype: "local",
    colNames: ['仓储ID','商品ID','串码详情','总积分','操作', '商品信息<span class="sm_false" onclick="set_sm(this);">扫码枪</span>','所属仓库','当前库存','辅助属性','商品品牌','商品编号','商品分类','规格型号','条形码','商品单位','库存预警','商品货位','赠送积分','零售名称','商品批次','商品串码','数量','零售单价','折扣<span onclick="set_discount();">（批量）</span>','零售金额','备注信息'],
    colModel: [
        {
            name: 'set_id',
            hidden:true
...
function add_goods(arr,page_type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        rows_info['warehouse'] = arr[i].warehouse; //所出仓库
        ...
        if(page_type){
            ...
            rows_info['price'] = arr[i].retail; //单价
            rows_info['discount'] = '1'; //折扣
            rows_info['total'] = arr[i].retail; //总价
            ...
            rows_info['discount'] = arr[i].discount; //折扣
    ...
    for (var i = 0; i < arr.length; i++) {
        if(arr[i].set_id !== '-1'){
            ...
            if(!regular_positive.test(arr[i].nums)){
                ...
                cal_row(arr[i].id,arr[i].nums,arr[i].price,arr[i].discount,arr[i].integral);//计算行数据
                return false;
                ...
                dump('第'+(i+1)+'行零售单价不正确');
                return false;
            }else if(!regular_discount.test(arr[i].discount) || (arr[i].discount-0)===0){
                dump('第'+(i+1)+'行折扣不正确');
                return false;
            }else{
                cal_row(arr[i].id,arr[i].nums,arr[i].price,arr[i].discount,arr[i].integral);//计算行数据
...
//计算表格数据
function cal_row(row_id,row_nums,row_price,row_discount,row_integral){
    var total=cal((row_nums-0)*(row_price-0)*(row_discount-0));//单价乘数量乘折扣
    $("#table").jqGrid('setCell',row_id,'total',total);//设置单商品总价格
    var all_integral=cal((row_nums-0)*(row_integral-0));//数量乘积分
    $("#table").jqGrid('setCell',row_id,'all_integral',all_integral);//设置单商品积分
    var set_nums = cal($("#table").getCol('nums', false, 'sum')); //数量合计
    var set_total = cal($("#table").getCol('total', false, 'sum')); //总价合计
    var set_integral = cal($("#table").getCol('all_integral', false, 'sum')); //积分合计
    $('#total').val(set_total); //单据金额
    $("#table").footerData("set", {"integral": "总积分:"+set_integral,"nums": "总数量:"+set_nums,"total": "总金额:"+set_total});//设置合计
}
