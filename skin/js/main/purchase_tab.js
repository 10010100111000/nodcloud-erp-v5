...
$("#table").jqGrid({
    datatype: "local",
    colNames: ['商品ID','所入仓库ID','辅助属性标识','串码商品','操作', '商品信息<span class="sm_false" onclick="set_sm(this);">扫码枪</span>', '辅助属性','所入仓库<span onclick="set_warehouse();">（批量）</span>','商品品牌','商品编号','商品分类','规格型号','条形码','商品单位','库存预警','商品货位','赠送积分','零售名称','商品串码','数量','购货单价','购货金额','商品批次','备注信息'],
    colModel: [
        {
            name: 'set_id',
            hidden:true
...
function add_goods(arr,page_type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        if(page_type){
            ...
            rows_info['price'] = arr[i].buy; //单价
            rows_info['total'] = arr[i].buy; //总价
        }else{
            //详情
