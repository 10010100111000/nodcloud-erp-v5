...
$("#table").jqGrid({
    datatype: "local",
    colNames: ['仓储ID','商品ID','串码详情','总积分','操作', '商品信息<span class="sm_false" onclick="set_sm(this);">扫码枪</span>','所属仓库','当前库存','辅助属性','商品品牌','商品编号','商品分类','规格型号','条形码','商品单位','库存预警','商品货位','赠送积分','零售名称','商品批次','商品串码','数量','退货单价','退货金额','备注信息'],
    colModel: [
        {
            name: 'set_id',
            hidden:true
    ...
    for (var i = 0; i < arr.length; i++) {
        if(arr[i].set_id !== '-1'){
            ...
            if(!regular_positive.test(arr[i].nums)){
                ...
                cal_row(cal_type,arr[i].id,arr[i].nums,arr[i].price,arr[i].integral);//计算行数据
                return false;
...
//计算行数据
function cal_row(cal_type,row_id,row_nums,row_price,row_integral){
    ...
    $('#integral').val(set_integral); //单据积分
