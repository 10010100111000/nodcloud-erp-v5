...
$("#table").jqGrid({
    datatype: "local",
    colNames: ['仓储ID','商品ID','串码详情','操作', '商品信息<span class="sm_false" onclick="set_sm(this);">扫码枪</span>','所属仓库','当前库存','辅助属性','商品品牌','商品编号','商品分类','规格型号','条形码','商品单位','库存预警','商品货位','赠送积分','零售名称','商品批次','商品串码','数量','备注信息'],
    colModel: [
        {
            name: 'set_id',
            hidden:true
...
function add_goods(arr,page_type){
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        if(page_type){
            ...
            rows_info['data'] = arr[i].data; //详情备注
        }
