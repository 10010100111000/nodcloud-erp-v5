$(function(){
    layui.use('table', function() {
        ...
        layui.table.render({
            ...
            even: ape_even,
            cols:  [ape_cols],
            url: '/index/service/repurchasebill_list',
            page: true,
...
function export_data() {
    ...
    alert_info('稍等，数据请求中', "/index/export/repurchase_bill?" + url_info,true);
}
//报表
        ...
        layer.ready(function() {
            var index = layer.open({
                ...
                success: function(layero, index) {
                    ...
                    //弹出后回调
                    $.post("/index/service/repurchasebill_info", {
                        "id": id,
                    }, function(re) {
                        $('#actual').val(re.class.actual);
                        $('#money').val(re.class.money);
                        for (var i = 0; i < re.bill.length; i++) {
                            $('#tabs').append('<tr><td>'+re.bill[i].time+'</td><td>'+re.bill[i].account.info.name+'</td><td>'+re.bill[i].money+'</td><td>'+re.bill[i].user.info.name+'</td><td>'+re.bill[i].data+'</td><td><button class="layui-btn layui-btn-primary layui-btn-sm" onclick="del_repurchasebill('+re.bill[i].id+','+re.bill[i].auth_info+')"><i class="layui-icon">&#xe640;</i></button></td></tr>');
                        }
                    });
                ...
                btn1: function(layero) {
                    ...
                    if(!regular_price.test(sum) || sum==="0"){
                        ...
                        $.post("/index/service/save_repurchasebill", {
                            "id": id,
                            "account": account,
                            "sum": sum,
                            "data": data
...
//删除资金详情
function del_repurchasebill(id,auth_type){
    if(auth_type){
        layui.use('layer', function() {
            layer.confirm('删除后资金将归还到原先账户，确定操作？', {
                ...
                $.post("/index/service/del_repurchasebill", {
                    "id": id
                }, function(re) {
                    if (re === "success") {
                        re_alert('删除成功!');
