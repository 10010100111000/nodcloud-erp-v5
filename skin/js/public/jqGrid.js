...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setColWidth = function () {
			...
			grid.width = ts.p.width-4;//nodcloud.com,重置宽度
...
$.jgrid.extend({
	...
	addRowData : function(rowid,rdata,pos,src) {
	    add_row++;//行条数自增
	    rowid='APE'+add_row;//nodcloud.com,优化传入
		if(!pos) {pos = "last";}
	...
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				var opt = $.extend({}, cm.editoptions || {} ,{id:iRow+"_"+nm,name:nm,ape:$t.rows[iRow].id});
				var elc = $.jgrid.createEl.call($t,cm.edittype,opt,tmp,true,$.extend({},$.jgrid.ajaxOptions,$t.p.ajaxSelectOptions || {}));
				
				$(cc).html("").append(elc).attr("tabindex","0");
				$.jgrid.bindEv.call($t, elc, opt);
				
				//nodcloud.com,单元格自定义内容选中
				if($(elc).get(0).tagName==='SPAN'){
			        if($(elc).find('input').length>0){
				        var tmp_t=$(elc).find('input').val();//原来input内容
				        window.setTimeout(function () { $(elc).find('input').val("").focus().val(tmp_t);},0);//选中前复制粘贴一下，光标定位到最后
				    }else if($(elc).find('select').length>0){
				        var select_tmp=$(elc).find("select").find('option:contains('+tmp+')');
				        if(select_tmp.length>0){
				            select_tmp.attr('selected', true);
				        }
				        window.setTimeout(function () {$(elc).find('select').focus()},0);
				    }
				}else{
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13 && !e.ctrlKey) {
						$($t).jqGrid("saveCell",iRow,iCol);//保存编辑内容
						//回车自动切换到下一个可编辑区域demos
		...
		return this.each(function (){
			...
			if(nCol !== false) {
				...
				if ($t.p.savedRow.length >0) {
				    ...
				    //nodcloud.com 当前行无可编辑区域，自动跳转下一行
