...
class Baksql {
    ...
    private $begin; //开始时间
    private $error;//错误信息
    public function __construct($hostname,$username,$password,$database,$path='skin/sql/') {
    ...
    public function get_filelist($Order = 0) {
        ...
        while (false !== ($filename = readdir($FilePath))) {
            if ($filename!="." && $filename!=".."){
                ...
                $FileAndFolderAyy[$i]['time_one'] = $this->getfiletime($filename);
                $FileAndFolderAyy[$i]['time_two'] = strtotime($FileAndFolderAyy[$i]['time_one']);
